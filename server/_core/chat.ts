...
const tools = (userId: number) => ({
  ...
  deleteBird: tool({
    description: "Permanently delete one bird from the registry. Only do this if the user explicitly asks to delete one specific bird. Use searchBirds to confirm the bird ID first. Never use this for bulk requests such as deleting all birds or multiple birds.",
    inputSchema: z.object({
