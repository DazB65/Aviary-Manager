...
const tools = (userId: number) => ({
  ...
  getShows: tool({
    description: "List the user's full show/exhibition records — every entry with its date, venue, species/class, group, result, notes, and which bird was shown. Optionally filter to one bird by birdId. Use this whenever the user asks about show results, where/when a bird was shown, or wants the details behind a show summary.",
    inputSchema: z.object({
      birdId: z.number().int().positive().optional().describe("Optional: only return shows for this bird"),
    }),
    execute: async ({ birdId }) => runLoggedReadTool(userId, "getShows", async () => {
      const [showRows, allBirds] = await Promise.all([
        birdId ? ShowService.getShowsByBird(birdId, userId) : ShowService.getShowsByUser(userId),
        BirdService.getBirdsByUser(userId),
      ]);
      const birdMap = new Map(allBirds.map((b: any) => [b.id, b]));
      const label = (id: number) => {
        const b = birdMap.get(id);
        return b ? (b.name || b.ringId || `Bird #${b.id}`) : `Bird #${id}`;
      };
      const summary = summariseShowResults(showRows);
      return {
        totalShows: summary.totalShows,
        wins: summary.wins,
    ...
    execute: async ({ birdId, name, ringId, cageNumber, gender, dateOfBirth, fledgedDate, colorMutation, genotype, speciesId, notes, fatherId, motherId }) => {
      try {
        ...
        if (fatherId !== undefined) {
          if (fatherId !== null) {
            const father = await BirdService.getBirdById(fatherId, userId);
            if (!father) return { success: false, error: `Father bird #${fatherId} not found` };
          }
          updates.fatherId = fatherId;
        }
        if (motherId !== undefined) {
          if (motherId !== null) {
            const mother = await BirdService.getBirdById(motherId, userId);
            if (!mother) return { success: false, error: `Mother bird #${motherId} not found` };
          }
  ...
  deleteBird: tool({
    description: "Permanently delete ONE bird from the registry. Use searchBirds to confirm the bird ID first, and pass the bird's name or ring ID in birdLabel so the approval card shows the user exactly which bird will be deleted. To delete several birds at once, use deleteBirds instead.",
    inputSchema: z.object({
      birdId: z.number().describe("ID of the bird to delete"),
      birdLabel: z.string().optional().describe("The bird's name or ring ID, shown on the approval card for confirmation"),
    }),
    needsApproval: true,
    execute: async ({ birdId }) => {
      try {
  ...
  deleteBirds: tool({
    description:
      "Permanently delete MULTIPLE specific birds in one approved action. Use this when the user asks to delete several named birds at once. First use searchBirds to resolve each bird to its numeric ID, then pass all the IDs in birdIds AND the matching names/ring IDs in birdLabels (same order) so the approval card shows the user exactly which birds will be deleted. Maximum 50 birds per call. Never use this to delete the user's entire collection without them clearly listing or confirming the birds.",
    inputSchema: z.object({
      birdIds: z.array(z.number()).min(1).max(50).describe("IDs of the birds to delete (1-50)"),
      birdLabels: z
        .array(z.string())
        .optional()
        .describe("Each bird's name or ring ID, in the same order as birdIds, shown on the approval card for confirmation"),
    }),
    needsApproval: true,
    execute: async ({ birdIds }) => {
      const uniqueIds = Array.from(new Set(birdIds));
      const deleted: Array<{ birdId: number; birdName: string }> = [];
      const failed: Array<{ birdId: number; reason: string }> = [];
      for (const birdId of uniqueIds) {
        try {
          const bird = await BirdService.getBirdById(birdId, userId);
          if (!bird) {
            failed.push({ birdId, reason: "not found" });
            continue;
          }
          const birdName = bird.name || bird.ringId || `#${bird.id}`;
          await BirdService.deleteBird(birdId, userId);
          deleted.push({ birdId, birdName });
        } catch {
          failed.push({ birdId, reason: "error" });
        }
      }
      logAiTool(userId, "deleteBirds", failed.length ? "error" : "success", {
